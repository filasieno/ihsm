...
Confirm **`packages/ihsm/package.json`**, **`packages/core/package.json`**, and **`packages/otel/package.json`** **`version`** values all match the tag you will push (`0.1.24` → tag `0.1.24`, no `v` prefix). Bump **patch +1** each release (`0.1.24` → `0.1.25`); stay on **`0.1.x`** — do not jump to **`0.2.0`** until a deliberate minor release. `@ihsm/core` depends on the same `ihsm` version; `@ihsm/otel` peer-depends on `ihsm` ≥ that version.
