...
id: 08-post-and-sync
slug: /tutorials/08-post-and-sync
---
...
Typed request/response: [Call services](/tutorials/10-call-services).
...
When the client needs a **return value**, use `call` — not `post` + `sync`. See [Call services](/tutorials/10-call-services).
