...
Compare with [Post & sync](/tutorials/08-post-and-sync): plain `this.post` from a handler is FIFO **after** the handler returns — `postNow` cuts ahead of those normal posts.
...
Use **`postNow`** to reuse existing protocol handlers as internal orchestration steps. Use **`then()`** for choice pseudo states ([then()](/tutorials/16-then)).
