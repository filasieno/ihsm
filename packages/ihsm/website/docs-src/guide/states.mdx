...
const counter = makeActor(CounterTop, { value: 0, step: 5 }, new Port());
...
const lamp = makeActor(LampTop, { brightness: 100, entryCount: 0 }, new Port());
await lamp.hsm.sync();
...
const media = makeActor(MediaTop, { trail: [] }, new Port());
