...
class Active extends GatewayTop {
  onEntry(): void {
    if (!this.ctx.link) {
      ...
      this.ctx.link = makeChildActor(asParentActor(this), LinkTop, linkCtx, new Port<typeof LinkTop>());
    }
...
const gw = makeActor(GatewayTop, { host: '' }, new Port<typeof GatewayTop>());
await gw.hsm.sync();
...
const order = makeActor(OrderTop, { amount: 80, limit: 100, phase: 'draft', notes: [] }, new Port());
