...
- **`makeActor` / `makeChildActor` / `makeTestActor`** — `port` is optional; omit it to use a default production `Port` (or `TestPort` in tests). Options can be passed as the third argument without an explicit port.

## [0.1.23] - 2026-06-20

### Changed

- Documentation: removed contributor callouts from the site home page and package README.

## [0.1.22] - 2026-06-20

### Changed

- Raised CI coverage thresholds and expanded runtime/instrumentation specs (port proxy paths, deferred timer enqueue, logger severities, collector fan-out).
...
- **CI / release** — removed Attic (`nix.1tdb.io`) from GitHub Actions; Nix setup uses `cache.nixos.org` only so workflows no longer fail when the self-hosted cache is unreachable. Release workflow publishes **`ihsm`** only (`@ihsm/core` deferred).
