...
Schedule a normal-priority [post](#post) after a wall-clock delay. **Handler-only.**

Backed by the machine's [Port](../classes/Port.mdx) timer service: when no custom port is supplied the
runtime instantiates a [Port](../classes/Port.mdx) whose `setTimeout`-based timer is used here. When
the timer fires, the event is enqueued like an ordinary `post`. This is intentionally **not**
on the external [Hsm](Hsm.mdx) / [Actor](../type-aliases/Actor.mdx) surface — clients schedule work via [post](#post).
...
Event name (same constraints as [post](#post))

##### eventPayload

...[`EventPayload`](../type-aliases/EventPayload.mdx)\<`Protocol`, `EventName`\>

Handler arguments tuple (same as [post](#post))
