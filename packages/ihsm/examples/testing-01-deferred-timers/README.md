...
one tick **every hour** using `hsm.defer(ms)`, which is backed by the machine's **standard port timer
...
- **No domain port:** the hourly follow-up is scheduled with `this.hsm.port.defer(HOUR_MS, 'onTick')`.
  `hsm.defer(ms)` delegates to the port timer service — a `Port` (real `setTimeout`) in
...
| **Test port** (`TestPort`) | A port test double that **records** what flows through it (`messages` / `events` / `trace`) and can `send` internal events inward. Use `advance(ms)` to fire due `hsm.defer(ms)` callbacks deterministically; wire `TestActor.subscribe` to `port.record` to trace every posted event. |
